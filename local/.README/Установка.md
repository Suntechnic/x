...
# для разработки
cp -R ../x.oceansites.ru/.git .
cp -R ../x.oceansites.ru/.gitmodules .
gitcheckout local
git submodule init
...
## Для проекта инициализируем .git

```sh
rm -rf images/

wget https://raw.githubusercontent.com/Suntechnic/x/refs/heads/main/.gi
tignore
...
## Для проекта устанавливаем сабмодули

Модуль модулей:
```bash
# git submodule add git@github.com:Suntechnic/xModule.git local/modules/x/module
git submodule add https://github.com/Suntechnic/xModule.git local/modules/x/module
```

Инструменты:
```bash
# git submodule add git@github.com:Suntechnic/xTools.git .dev/tools
git submodule add https://github.com/Suntechnic/xTools.git .dev/tools
```

Инструкции для Ai:
```bash
# git submodule add git@github.com:Suntechnic/vs-instructions.git .github/instructions
git submodule add https://github.com/Suntechnic/vs-instructions.git .github/instructions
```

Промпты для Ai:
```bash
# git submodule add git@github.com:Suntechnic/prompts.git .github/prompts
git submodule add https://github.com/Suntechnic/prompts.git .github/prompts
```
...
https://github.com/yngc0der/bitrix-rabbitmq  
...
1 В /local добавляем файл .settings_extra.php:
```php
<?
...
2 Так же сюда можно добавить действия которые необходимо выполнить до загрузки ядра, например переключение языков, в зависимости от домена.
Например:
```php
